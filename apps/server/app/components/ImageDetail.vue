...
const image: ComputedRef<BlobObject> = computed(() => {
  return images.value!.filter(
    (file: BlobObject) => file.pathname.split(".")[0] === route.params.slug![0]
  )[0]!;
});
...
onKeyStroke("ArrowLeft", () => {
  if (isFirstImg.value) router.push("/");
    ...
    router.push(
      `/detail/${images.value[currentIndex.value - 1]?.pathname.split(".")[0]}`
    );
});
...
async function saveImage() {
  if (filterUpdated.value && imageEl.value) {
    ...
    await uploadImage(imageToUpload, true).finally(
                ...
                <div class="group">
                  <img
                    ...
                    :class="[
                      {
                        imageEl:
                          route.params.slug[0] === image.pathname.split('.')[0],
                      },
