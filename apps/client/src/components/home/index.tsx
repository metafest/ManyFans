...
export function Home() {
  ...
  if (!isMounted) return <div className="min-h-screen bg-white dark:bg-black" />;

  return (
    <div className={`flex flex-col items-center ${currentTheme === 'dark' ? 'dark' : ''}`}>
      <BackgroundLines
        svgOptions={{ duration: 7 }}
        className="flex items-center justify-center w-full flex-col px-4"
      >
        <h2 className={`bg-clip-text text-transparent text-center ${headingGradient} text-2xl md:text-4xl lg:text-7xl font-sans py-2 md:py-10 relative z-20 font-bold tracking-tight`}>
          ManyFans!
          <br />
        </h2>
        <p className={`max-w-xl mx-auto text-sm md:text-lg ${paragraphTextColor} text-center`}>
